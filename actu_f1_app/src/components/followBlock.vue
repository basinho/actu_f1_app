<template>
  <div class="circleDiv" @click="goToPilotPage">
    <img v-if="img" :src="img" :alt="firstname" />
    <div class="follow-indicator" :class="{ followed: isFollowed }" @click.stop="toggleFollow">
      {{ isFollowed ? '-' : '+' }}
    </div>
  </div>
</template>
...
const goToPilotPage = () => {
  router.push({ name: 'driverPage', params: { pilotId: props.id } });
};
